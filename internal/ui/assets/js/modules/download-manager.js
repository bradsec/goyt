...
export class DownloadManager {
    ...
    async handlePlaylistDownload(event) {
        ...
        try {
            ...
            if (type === 'playlist') {
                ...
                await this.apiClient.startPlaylistDownload(downloadData.url, downloadData.type, downloadData.quality, downloadData.format);
                ...
                await this.apiClient.downloadFirstVideo(downloadData.url, downloadData.type, downloadData.quality, downloadData.format);
    ...
    updateItemInPlace(el, download, sectionKey, index) {
        ...
        if (statsEl)
            statsEl.innerHTML = this.buildProgressStats(download, bounded, stage);
    ...
    renderFileSize(download) {
        ...
        if (!terminal.includes(download.status) || !download.file_size)
            return '';
    ...
    renderFFMPEGProgress(progress) {
      ...
      `);
        ...
        if (progress.video_codec || progress.audio_codec || progress.resolution) {
            const mediaDetails = [];
            if (progress.video_codec)
                mediaDetails.push({ label: 'Video Codec', value: progress.video_codec });
            if (progress.audio_codec)
                mediaDetails.push({ label: 'Audio Codec', value: progress.audio_codec });
            if (progress.resolution)
                mediaDetails.push({ label: 'Resolution', value: progress.resolution });
            if (progress.fps && progress.fps !== '')
                mediaDetails.push({ label: 'FPS', value: progress.fps });
        ...
        if (progress.current_frame > 0 || progress.bitrate || progress.processing_time) {
            ...
            if (progress.bitrate)
                processingDetails.push({ label: 'Bitrate', value: progress.bitrate });
            if (progress.processing_time)
                processingDetails.push({ label: 'Processing Time', value: progress.processing_time });
